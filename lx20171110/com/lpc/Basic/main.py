    ...
    def main(self=None):
        ...
        while True:
            ...
            if userchoice == "1":
                ...
                scgw.zhuce(username=username, password=userpass, userpass2=userpass2, yhxxpath=yhxxpath, cs=login_limit)
                continue
            elif userchoice == "2":
                print("欢迎使用登录功能")
                scgw.login(yhxxpath=yhxxpath, gwqdpath=gwqdpath, cs=login_limit)
                continue
            elif userchoice == "3":
                czje = int(input("请输入要充值的金额\t\t"))
                scgw.chuzhi(username=scgw.IS_LOGIN, je=czje, yhxxpath=yhxxpath)
                continue
            elif userchoice == "4":
